...
def read_form_csv():
  res = []
  f=open('000001SZ_Vol_2014.csv')
  df=pd.read_csv(f)
  price = np.array(df['price'])
  volume = np.array(df['volume'])
  price = make_input_norm(price)
  volume = make_input_norm(volume)
  print(price)
  print(volume)
  for i in range(len(price) - time_step):
      ...
      for j in range(time_step):
          ...
          tmp.append(volume[i + j])
      res.append(tmp.copy())
  return np.array(res)

def make_input_norm(X_data):
    mean = np.mean(X_data,axis=0)
    variance = np.var(X_data,axis=0)
    X_data -= mean
    #The 1e-9 avoids dividing by zero
    X_data /= np.sqrt(variance)+1e-9
    return X_data
...
def load_data():
  ...
  X_train = np.reshape(X_train, (-1,time_step,input_size))
  X_test = np.reshape(X_test, (-1,time_step,input_size))
  return X_train,X_test,y_train,y_test
...
class Model():
  def __init__(self,config):
    ...
    """Place holders"""
    self.input = tf.placeholder(tf.float32, [None, time_step,input_size], name = 'input')
    ...
    with tf.name_scope("input_layer") as scope:
      with tf.variable_scope("input_layer_params"):
        input_layer_w = tf.get_variable("input_layer_w", [input_size, 1])
        input_layer_b = tf.get_variable("input_layer_b", [1])
      input1=tf.reshape(self.input,[-1,input_size])
      self.input_rnn=tf.matmul(input1,input_layer_w)+input_layer_b
      self.input_rnn=tf.reshape(self.input_rnn, (-1, sl))
    ...
    #!!!modify
    input_list = tf.unstack(tf.expand_dims(self.input_rnn,axis=2),axis=1)
