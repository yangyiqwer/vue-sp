...
X_train = np.reshape(X_train, (-1,time_step,1,input_size))
X_test = np.reshape(X_test, (-1,time_step,1,input_size))
...
x = tf.placeholder("float", shape=[None, D, 1, input_size], name = 'Input_data')
...
with tf.name_scope("Reshaping_data") as scope:
    x_image = tf.reshape(x, [-1,D,1,input_size])
...
with tf.name_scope("Conv1") as scope:
    W_conv1 = weight_variable([4, 1, input_size, num_filt_1], 'Conv_Layer_1')
...
while(time < 1):
    ...
    with tf.Session() as sess:
        ...
        acc_test = result[0]
        ans.append(acc_test)
        dic[int(acc_test * 100)] += 1
        #print('The network has %s trainable parameters'%(result[1]))
        saver=tf.train.Saver(tf.global_variables())
        if(acc_test > best):
            best = acc_test
            model_datapath = os.path.join(os.getcwd(), 'cnn_model')
