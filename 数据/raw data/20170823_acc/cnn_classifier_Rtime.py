...
while(un_finish):
  ...
  with tf.Session() as sess:
      ...
      for i in range(max_iterations+1):#training process
  ...
  if(test_acc[-1] > 0.59):
    un_finish = False
  print("time:" + str(time) + ":" + str(test_acc[-1]))
for i in range(101):
  if(dic[i] != 0):
...
print(np.shape(train_acc))
print(np.shape(test_acc))
newdf['time'] = np.array(time_s)
newdf['train_acc'] = np.array(train_acc)
newdf['test_acc'] = np.array(test_acc)
newdf.to_csv("acc.csv", index = False)
